...
// ── Mobile: replay toggle via double-tap pause button ────────────────────────
...
class $modify(ToastyMobilePauseLayer, PauseLayer) {
    void customSetup() {
        ...
        auto makePill = [&](const char* label, ccColor3B color, float yPos, SEL_MenuHandler sel) {
            ...
            bg->setContentSize({ 120.f, 38.f });
            bg->setColor(color);
            auto* lbl = CCLabelBMFont::create(label, "bigFont.fnt");
            lbl->setScale(0.45f);
            lbl->setPosition({ 60.f, 19.f });
            bg->addChild(lbl);
            auto* item = CCMenuItemSpriteExtra::create(bg, this, sel);
            auto* menu = CCMenu::create();
            menu->addChild(item);
            menu->setPosition({ winSize.width / 2.f, yPos });
            addChild(menu, 10);
        };

        if (playbackActive) {
            makePill("Stop Replay", { 200, 60, 60 }, 46.f,
                menu_selector(ToastyMobilePauseLayer::onStopReplay));
        } else if (engine->hasMacroInputs()) {
            makePill("Start Replay", { 30, 180, 170 }, 46.f,
                menu_selector(ToastyMobilePauseLayer::onStartReplay));
        }

        if (captureActive) {
            makePill("Stop Capture", { 200, 60, 60 }, captureActive && !playbackActive ? 46.f : 90.f,
                menu_selector(ToastyMobilePauseLayer::onStopCapture));
        } else if (!playbackActive) {
            makePill("Start Capture", { 80, 80, 200 }, engine->hasMacroInputs() ? 90.f : 46.f,
...
// ── Mobile: first-run notification says "tap the button" ─────────────────────
